...
public class Window extends JFrame {
    ...
    public Window() {
        ...
        JScrollPane scrollPane = new JScrollPane(dataSheetTablePanel);
        ...
        saveButton.addActionListener(e -> {
            ...
            if (fileChooser.showDialog(null, "Save") == JFileChooser.APPROVE_OPTION) {
                ...
                if (!directory.exists()) {
                    directory.mkdirs();  // Створює директорію, якщо вона не існує
                }
                try {
                    File saveFile = new File(directory, getLastPathName());
                    dataSheetTablePanel.getDataSheetTable().getModel().getDataSheet().saveDocument(saveFile);
                } catch (FileNotFoundException | TransformerException e1) {
                    JOptionPane.showMessageDialog(null, "Error saving file: " + e1.getMessage(), "Save Error", JOptionPane.ERROR_MESSAGE);
                    e1.printStackTrace();
                }
            }
        });
